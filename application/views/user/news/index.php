			...
			<div class="content-body block">
				...
				<div class="setup-form block">
					...
					<form action="" method="post" class="m-t-lg">
                        ...
                        <a href="<?php echo HOST_URL.$ctrlUrl?>/status/<?php echo $this->encrypt->encode($value->news_id)?>/<?php echo ($value->show_status == 1) ? 'N' : 'Y';?>" class="left <?php echo ($value->show_status == 0) ? 'icon-edit' : 'icon-delete';?> block" style="margin-right :5px">
													<i class="<?php echo ($value->show_status == 1) ? 'ion-close-round' : 'ion-checkmark-round';?>"></i>
												</a>

												<a href="<?php echo HOST_URL.$ctrlUrl?>/edit/<?php echo $this->encrypt->encode($value->news_id)?>" class="left icon-edit block">
													<i class="ion-edit"></i>
												</a>

												<a onclick="javascript: return confirm('Do you want to delete this entry?');" href="<?php echo HOST_URL.$ctrlUrl?>/delete/<?php echo $this->encrypt->encode($value->news_id)?>" class="left icon-delete block">
