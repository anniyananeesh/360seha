		...
		<div class="setup-body container-fluid text-center bg-grayed">
          ...
          <p class="h2 block strong"><?php echo lang('auto.offers');?></p>
                ...
                <div class="clear"></div>
								...
								<div class="col-12">
									<div class="setup-form-group block">
										<label class="setup-form-label block"><?php echo lang('auto.department');?> <span class="required">*</span></label>
                    <select class="setup-form-control setup-form-control-sm block <?php echo (form_error('department')) ? 'error-border' : ''?>" id="department" name="department" <?php echo (form_error('department')) ? 'title="'.form_error('department').'"' : ''?>>
                      <option value=""><?php echo lang('auto.choose')?></option>
                      <?php foreach($departments AS $key => $value):?>
                        <option value="<?php echo $value->id?>" <?php echo ($value->id == $post['department']) ? 'selected' : ''?>><?php echo $value->{title_.$lan}?></option>
                      <?php endforeach;?>
                    </select>
									</div>
								</div>
                ...
                <div class="clear"></div>
								...
								<div class="col-5">
									<div class="setup-form-group block">
										<label class="setup-form-label block"><?php echo lang('auto.voucher_code');?> </label>
										<input type="text" class="setup-form-control setup-form-control-sm block <?php echo (form_error('voucher_code')) ? 'error-border' : ''?>" <?php echo (form_error('voucher_code')) ? 'title="'.form_error('voucher_code').'"' : ''?> value="<?php echo (isset($post['voucher_code'])) ? $post['voucher_code']: '' ?>" id="voucher_code" name="voucher_code"/>
									</div>
								</div>
                ...
                <div class="clear"></div>
                    ...
                    <?php if($image != null):?>
                        <div class="block" style="margin-bottom: 15px;">
                            <img src="<?php echo $imageShowPath?><?php echo $image?>" width="200"/>
                        </div>
                    <?php endif;?>
                    <div class="clearfix"></div>
										<label class="setup-form-label block">Upload image (640px X 640px). Maximum 1Mb filesize <span class="required">*</span></label>
