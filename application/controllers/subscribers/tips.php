...
class Tips extends Account_Controller{
    ...
    {
        parent::__construct(); 
        $this->view = SUBSRIBER_URL.'tips';
        $this->load->model('subscribers/tips_model','modelNameAlias');
    }

    public function index()
    {
        $this->data['content'] = $this->view.'/index';
        $this->data['title'] = 'Health Tips Page';
        $this->data['sub_title'] = 'Manage your Health Tips here';
        $where = array($this->table.'.subscriber' => $this->encrypt->decode($this->user['user_id']));
        $this->data['tips'] = $this->modelNameAlias->fetchAll($where);
    ...
    public function add()
    {
        ...
        $this->data['title'] = 'Add new health tip';
        $this->data['sub_title'] = 'Manage your Health Tips page';
        $error = false;
        ...
        if($this->input->post())
        {
            ...
            if($this->form_validation->run() == TRUE)
            {
                $save = array(
                    ...
                    $this->table.'.tips_title' => $this->input->post('title'),
                    $this->table.'.tips_title_ar' => $this->input->post('title_ar'),
                    $this->table.'.tips_description' => $this->input->post('description'),
                    $this->table.'.tips_description_ar' => $this->input->post('description_ar'),
                ...
                $tips_id = $this->modelNameAlias->save($save);

                if($this->input->post('send'))
                {
                    $this->send_notification((int) $this->encrypt->decode($this->user['user_id']), $this->input->post());
                    $save = array($this->table.'.send' => 1);
                    $where = array($this->table.'.tips_id' => $tips_id);
    ...
    { 
        ...
        $this->data['title'] = 'Update health tip';
        $this->data['sub_title'] = 'Manage your health tip page';
        ...
        if($this->input->post())
        {
            ...
            if($this->form_validation->run() == TRUE)
            {
               $save = array(
                ...
                $this->modelNameAlias->save($save,$where);
                $this->session->set_flashdata('message', 'Health tips updated!');
                redirect($this->data['ctrl_url']);
            }
        }
    ...
    public function send($id)
    {
        ...
        if(!$record)
        {
            ...
            $news = array();
            $news['title'] = $record->tips_title;
            $news['description'] = $record->tips_description;

            $save = array($this->table.'.send' => 1);
            $where = array($this->table.'.tips_id' => $id);
            $this->modelNameAlias->save($save, $where);
    ...
    public function delete($id)
    {
        ...
        if(!$record)
        {
            ...
            $where = array($this->table.'.tips_id' => $id);
            unlink(UPLOAD_FOLDER.'subscribers/tips/'.$record->image_url);
            unlink(UPLOAD_FOLDER.'subscribers/tips/'.$record->thumb_url);
    ...
    {
        $config['upload_path'] = UPLOAD_FOLDER.'subscribers/tips/';
        ...
        if (isset($_FILES['userfile']) && !empty($_FILES['userfile']['name']))
        {
            if ($this->upload->do_upload('userfile'))
            {
                ...
                $img = WideImage::load(UPLOAD_FOLDER.'subscribers/tips/'.$_POST['userfile']);
                
                $resized = $img->resize(285, 285);
                $resized->saveToFile(UPLOAD_FOLDER.'subscribers/tips/'.$_POST['thumbfile']);
                return true;
            }
            else
            {
